...
def render_weekly_snapshot(user_name: str, data: dict) -> str:
    ...
    """
    top_rows = "\n".join(
        f"<tr><td style='padding:6px 0'>{html.escape(e['category'])}</td>"
    ...
    goals_rows = "\n".join(
        f"<li style='margin:4px 0'>{html.escape(g['name'])} <strong>+{_fmt_money(g['delta'])}</strong></li>"
    ...
    insight = html.escape(data.get("insight") or "")
    insight_block = (
    ...
    inner = f"""
      <p style='font-size:14px'>Hola {html.escape(user_name)}, este es tu resumen de <strong>{html.escape(data.get('week_label', 'la semana'))}</strong>.</p>
...
def render_smart_alert(user_name: str, headline: str, body_html: str) -> str:
    inner = f"""
      <p style='font-size:14px'>Hola {html.escape(user_name)},</p>
