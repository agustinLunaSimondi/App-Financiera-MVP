...
import { Wallet, ArrowRight, ArrowLeft, Loader2, Eye, EyeOff, AlertCircle } from 'lucide-react';
...
export function ResetPasswordPage() {
    ...
    if (!token) {
        return (
            <div className="min-h-screen flex items-center justify-center p-4 bg-zinc-50 dark:bg-zinc-950">
                <div className="max-w-md w-full glass-card p-8 rounded-3xl text-center space-y-4">
                    <AlertCircle className="w-10 h-10 text-amber-500 mx-auto" />
                    <p className="text-sm text-zinc-500 dark:text-zinc-400">
                        Este link de recuperación no es válido. Pedí uno nuevo.
                    </p>
                    <Link to="/forgot-password" className="inline-flex items-center gap-2 text-sm font-black text-emerald-600 dark:text-emerald-400 hover:underline underline-offset-4">
                        Pedir un link nuevo
                    </Link>
                </div>
            </div>
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(parseApiError(err, 'El link expiró o no es válido. Pedí uno nuevo.'));
            setIsLoading(false);
        }
    };
            ...
            >
                <div className="text-center space-y-2">
                    ...
                    <p className="text-zinc-500 dark:text-zinc-400 font-medium text-sm">
                        Mínimo 8 caracteres. Cerramos tus otras sesiones por seguridad.
                    </p>
                </div>
                ...
                <form className="space-y-4" onSubmit={handleSubmit} noValidate>
                    ...
                    <div className="space-y-2">
                        <label htmlFor="reset-password" className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 ml-4 block">Nueva contraseña</label>
                    ...
                    <div className="space-y-2">
                        <label htmlFor="reset-confirm-password" className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 ml-4 block">Confirmar contraseña</label>
                        <input
                    ...
                    >
                        {isLoading ? (
                            ...
                            <>
                                Guardar y entrar <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
                            </>
                        )}
                    </button>
